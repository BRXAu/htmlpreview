...
function allDocIsReady() {
	...
	TweenLite.set(logos, {x:350})

	//Frame 1
	theTimeline.add(new TweenLite.from('#copy1_f1_mask', 0.5, {width:'0px', ease:Power2.easeOut}));
	theTimeline.add(new TweenLite.from('#copy2_f1', 0.5, {y:-61, ease:Power2.easeOut}));

	//Frame 2
	theTimeline.add(new TweenLite.from('#imgBall', 0.8, {scale:0, ease:Power0.easeIn, onStart:ballSpins}),"+=0.5")
	theTimeline.add(new TweenLite.from('#imgGoalposts_right, #imgGoalposts_left', 1.5, {x:300, ease:Linear.easeNone}), "-=0.5");		
	theTimeline.add(new TweenLite.to('#imgBall', 1, {scale:2, x:-48, y:-7, zIndex:2, ease:Power0.easeOut}), "-=0.5")	

	theTimeline.add(new TweenLite.to('#copy1_f1_mask, #copy2_f1_mask', 0.15, {opacity:0}), "-=1.50")

	theTimeline.add(new TweenLite.to('#f1', 0.5, {opacity:0}), "+=0.5")

	//Frame 3
	theTimeline.add(new TweenLite.from('#copy1_f2_mask', 0.5, {width:'0px', ease:Power2.easeOut}));
	theTimeline.add(new TweenLite.from('#copy2_f2', 0.5, {y:-36, ease:Power2.easeOut, onComplete:rollInAwards}));

	theTimeline.add(new TweenLite.to('#copy1_f2_mask, #copy2_f2_mask', 0.30, {opacity:0}), "+=0.8");

	theTimeline.add(new TweenLite.to(logos, 0.5, {opacity:0}), "+=2.5")

	//Frame 4
	theTimeline.add(new TweenLite.from('#copy2_mask', 0.5, {width:'0px', ease:Power2.easeOut}));
	theTimeline.add(new TweenLite.from('#copy3', 0.5, {y:-73, ease:Power2.easeOut, delay:-0.2}));
	...
	function rollInAwards() {
		...
		logos.forEach(function(logos, i) {
			...
			const baseDelay = 0.5;
			const delayOffset = isFirstRow ? 0 : 0.15;
			const staggerDelay = rowIndex * 0.02;

			TweenLite.fromTo(logos, 2, {
				x:360 - paddingX, rotation: angle
			}, 
			{
