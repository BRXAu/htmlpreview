(function() {
  ...
  const legs = [0, quarter, quarter * 2, duration - quarter];
  const masks = ['01', '02', '03', '04'];
  ...
  tl.to('.brx-svg-loader-wrapper #svgContainer', 0.2, { autoAlpha: 1 }, delay);
  delay += 0.2;
  tl.to('.brx-svg-loader-wrapper #text_supersmart', 0.4, { autoAlpha: 1 }, delay);
  delay += 0.8;
  tl.to('.brx-svg-loader-wrapper #text_worthbuffer', 0.4, { autoAlpha: 1 }, delay);
  delay += 1;
  tl.to('.brx-svg-loader-wrapper #introText', 0.3, { opacity: 0 }, delay);
  delay += 0.3;
  tl.to('.brx-svg-loader-wrapper #loader', 0.3, { autoAlpha: 1 }, delay)
  delay += 0.3;
  tl.call(() => {
    loadDelay = 1;
  });

  masks.forEach((id, index) => {
    tl2.to(`.brx-svg-loader-wrapper  #mask${id}`, quarter, { scaleY: 1, ease: 'none' }, legs[index]);
    tl2.to(
      `.brx-svg-loader-wrapper  #mask${id}a`,
      quarter,
      { scaleY: 0, transformOrigin: '100% 100%', ease: 'none' },
      legs[index]
      );
  });
  ...
  const scrubSteps = [
    { progress: 0.4, duration: duration },
    { progress: 0.8, duration: duration * 2 },
    { progress: 0.9, duration: duration * 3 },
    { progress: 0.98, duration: duration * 5 }
  ];

  for (let i = 0; i < scrubSteps.length; i++) {
    scrubs[i] = tl.to(
      tl2,
      {
        progress: scrubSteps[i].progress,
        duration: scrubSteps[i].duration,
        ease: Linear.easeNone
      },
      delay
      )

    delay += scrubSteps[i].duration
  }

  const loaded = isFullyLoaded();
  if (!isFullyLoaded()) {
...
function fullyLoaded(scrub1,scrub2,scrub3,scrub4,tl2){
  scrub1.pause();
  scrub2.pause();
  scrub3.pause();
  scrub4.pause();

  TweenMax.to(tl2, 0.5, { progress: 1, ease: Linear.easeNone });
  TweenMax.to(".brx-svg-loader-wrapper #under,.brx-svg-loader-wrapper #fadeOut", 0.3, { autoAlpha: 0, ease: Linear.easeNone, delay:0.4 });
  TweenMax.to(".brx-svg-loader-wrapper #loader-wrapper", 0.3, { autoAlpha: 0, ease: Linear.easeNone, delay:1});
  TweenMax.delayedCall(1.3, removeElement, ['tempLoaderContainer'])
}

function callWithRandomDelay(fn,delayNum, ...args) {
  const delaySeconds = delayNum
  ...
  function startSpeedTest(){
    let userImageLink = "upload.png"; 
    let downloadSize = 5616998; 

    let downloadImgSrc = new Image(); 
    downloadImgSrc.onload = function () { 
      estimateSpeed(time_start,downloadSize); 
    };
  ...
  function estimateSpeed(time_start,downloadSize) { 
  ...
  function startLoader(speedInMbps){
    let duration;
    if(speedInMbps <= 10){
      duration = randomInRange(6,8)
    }
    else if((speedInMbps > 10)||(speedInMbps < 40)){
      duration = randomInRange(5,3)
    }
    else {
      duration = randomInRange(3,1)
    }
    startAnimation(duration);
  }

  // Global function that is called 
  function startCustomSVGLoader(){
    appendToBody(inlineSVGContent);
    startSpeedTest();
  }
  window.startCustomSVGLoader = startCustomSVGLoader;

})();

document.addEventListener('DOMContentLoaded', function() {
  startCustomSVGLoader()
});
